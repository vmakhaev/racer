...
, events: {
  ...
  , init: function (model) {
      model._txnRegistry = new TransactionRegistry(idGenerator);
      var createdCount = 0;
      function idGenerator () {
        return model.clientId + createdCount++;
      }
      this._ops = {
        // <pathTodoc>: {
        //   pending: []
      ...
      incoming.on('txnOk', function (op) {
      });

      return
      // TODO Clean up below

      var specCache = model._specCache = new SpecCache();

      model.numTxnsCreated = 0;

      var registry = model._txnRegistry = new TransactionRegistry(model._clientId);
      registry.on('remove', function () {
        specCache.invalidate();
      });

      var memory = model._memory;
      model.onTxn = function (txn) {
        if (model._clientId === transaction.getClientId(txn)) {
          model.onLocalTxn(txn);
        } else {
          model.onRemoteTxn(txn);
        }
      };

      model.onRemoteTxn = function (txn) {
      };

      model.onLocalTxn = function (txn) {
      };
  ...
  , middleware: function (_model, middleware) {
      ...
      middleware.txn.add(broadcastTxn);
//      middleware.txn.add(commitTxn);
      ...
      function broadcastTxn (req, res, next) {
        var txn = req.data;
        req.model.broadcaster.emit('txn', txn);
        return next();
      }
...
, proto: {
  ...
  , _nextTxnId: function () {
      return this._clientId + this.numTxnsCreated++;
    }
