...
, events: {
    init: function (model) {
      ...
      function pendingToInflight (txnId) {
        var timeouts = model._ackTimeouts;
        var timeout = timeouts[txnId];
        if (timeout) {
          clearTimeout(timeout);
          delete timeouts[txnId];
        }
        ...
        model.broadcastTxns(txnsToSend);
      }
...
, proto: {
  ...
  , broadcastTxns: function (txns) {
      var timeouts = this._ackTimeouts = {};
      this.broadcastTxns = function (txns) {
        var lastTxn = txns[txns.length-1];
        var lastTxnPath = transaction.getPath(lastTxn);
        var timeout = timeouts[pathToDoc(lastTxnPath)];
        if (timeout) throw new Error('Unexpected');
