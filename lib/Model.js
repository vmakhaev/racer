...
function Model (init) {
  ...
  // It's inverse is this.receiveEmitter, which handles events from the outside
  // world.
  var broadcaster = this.broadcaster = new EventEmitter;
  ...
  // Pause the readStream by default, until a socket-like stream is open as the
  // destination stream.
  this.readStream.pause();

  // Define a convenience method for declaring which channels we want to
  // automatically and periodically re-send messages until we cleanup the
  // timers via typically incoming messages or invoking the inverse method of
  // the one that generated the message in the first place.
  //
  // Example:
  ...
  //     });
  var timeouts = {};
  broadcaster.shouldResend = function (channel, opts) {
    broadcaster.on(channel, function () {
      if ('timeout' in opts) throw new Error('Unexpected');
      var args = Array.prototype.slice.call(arguments);
      var hash = opts.broadcastHash.apply(null, args);
      args.unshift(channel);
      timeouts[hash] = setTimeout( function () {
        delete timeouts[hash];
        broadcaster.emit.apply(broadcaster, args);
      }, opts.every);
    });
    var until = opts.until;
    for (var i = until; i--; ) {
      incoming.on(until[i], cleanup);
    }
    function cleanup () {
      var hash = opts.untilHash.apply(null, arguments);
  ...
  var incoming = this.incoming = emitStream(this.writeStream);
