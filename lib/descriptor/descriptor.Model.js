...
, proto: {
  ...
  , unfetch: function (/* descriptors... */) {
      ...
      for (var i = targets.length; i--; ) {
        var target = targets[i];
        var updatedCounts = readable.decrReadCount(target, docReadCounts);
  ...
  , _onReadAck: function (data, targetIndex) {
      ...
      for (var targetHash in pointers) {
        ...
        while (cb = callbacks.shift()) {
          var scopedPath = readable.resultPath(readableMetadata.target);
          var scopedResult, keyPath;
          if (ptr === true) {
            var starIndex = targetHash.indexOf('*');
            scopedResult = (starIndex === -1)
                         ? this.at(targetHash)
                         : this.at(targetHash.substring(0, starIndex - 1));
          } else if ('id' in ptr) {
...
function prepReadTargets (targets, targetIndex, cb, sendIfAlreadyLocal, model) {
  ...
  for (var i = targets.length; i--; ) {
    ...
    if (! (targetHash in targetIndex)) {
      ...
      var msg = readable.fetchMsg(target, model._fetches, model._subscriptions, model._memory._versions);
      toSend.push(msg);
    ...
    // co-incides with a prior subscribe, then...
      ...
      // Or we do have a result from a prior read
        ...
        if (sendIfAlreadyLocal) {
          if (cb) readableMetadata.callbacks.push(cb);
          toSend.push(readable.fetchMsg(target, model._fetches, model._subscriptions, model._memory._versions));
//          toSend.push({
